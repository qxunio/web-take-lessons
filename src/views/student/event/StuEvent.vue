    ...
    <div v-if="this.eventModel.id !== undefined">
      ...
      <div class="page-container-title-right" style="padding-bottom: 20px;padding-top: 20px">
        <el-tooltip class="item" effect="dark" :manual=true v-model="showRefreshTip" content="推荐使用刷新按钮~"
                    placement="left">
          <el-button icon="el-icon-refresh" type="success" :loading="loadingPage" circle title="刷新" @click="list"></el-button>
        </el-tooltip>
      </div>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="operate">
              ...
              <a @click="create(scope.row.subjectId)">关注</a>
            </div>
          </template>
        </el-table-column>
      </el-table>
...
import {stuEventListConfigSubject, stuFocusCreateConfigSubject, stuLock} from "@/api/api";
...
@Component
export default class StuEvent extends Vue {
  ...
  list() {
    ...
    stuEventListConfigSubject({eventId: this.eventModel.id}).then((res: ResultData) => {
   ...
   * 创建
   */
  create(configSubjectId: string) {
    if (VerifyUtil.isStringBlank(configSubjectId) || VerifyUtil.isStringBlank(this.eventModel.id)) {
      this.$notify({
        title: '创建关注错误',
        message: '参数缺失',
        type: 'warning',
    ...
    let data = {
      ...
      configSubjectId: configSubjectId
    }
    stuFocusCreateConfigSubject(data).then((res: ResultData) => {
      if (ResultUtil.success(res)) {
        console.log(res)
        this.$notify({
          title: '关注成功',
